...
function makeShell(player,excluded=[],liveItems=[]) { const items=requireItems(diverseSmallItems(4,excluded,item=>item.price>=20&&item.price<=500,liveItems),4,"Shell Game small-prize bank"), shown=items.map((x,i)=>x.price+(i%2?-8:9)), bonus=pick(requireItems(grandPrizeCandidates(excluded,liveItems),1,"Shell Game grand-prize bank")),bonusPrize={...prizeIntro(bonus),announcerText:prizeAnnouncementText(bonus,"You could win this spectacular grand prize! It's")}; return {...base("shellGame","SHELL GAME",player,"Price four small prizes. Every correct higher-or-lower decision wins that prize and earns a marker for one shell. Then choose from the shells you earned. Find the hidden ball to win the grand prize. Earn all four shells and the grand prize is guaranteed.",[bonusPrize,...items.map(prizeIntro)]),bonusPrize,_bonusPrice:bonus.price,items:items.map((x,i)=>({...prizeIntro(x),shownPrice:shown[i]})),_prices:items.map(i=>i.price),itemIndex:0,shells:0,wonSmallValue:0,_ball:Math.floor(Math.random()*4),chosenShells:[],prompt:`${items[0].name}: higher or lower than ${money(shown[0])}?`,mode:"choice",options:["Higher","Lower"],stage:"prices"}; }
