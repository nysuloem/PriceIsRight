...
const prizeIntro = (p) => {
  ...
  return { id, imageKey:`${id}|${image||"no-photo"}`, name:p.name, brand:p.brand, description:p.description, image, imageAlt:p.imageAlt||p.name, visual:productVisual(p), category:itemCategory(p), imageVerified:verifiedImage(p), announcerText:`It's ${p.brand}'s ${p.name}! ${p.description}` };
};
const carIntro = (p) => ({...prizeIntro(p),visual:"🚗",announcerText:`IT'S A NEW CAR!!! It's the ${p.name}! ${p.description}`});
...
function makePunch(player, excluded=[],liveItems=[]) {
  const prizes=diverseSmallItems(3,excluded,()=>true,liveItems); const shown=prizes.map((p,i)=>p.price+(i%2?9:-8));
...
function makeGrocery(player, excluded=[],liveItems=[]) {
  ...
  const rawBonus=pick(requireItems(grandPrizeCandidates(excluded,liveItems),1,"Grocery Game grand-prize bank")),bonusPrize={...prizeIntro(rawBonus),announcerText:`You're playing for this fabulous grand prize: ${rawBonus.description}`};
...
function makeShell(player,excluded=[],liveItems=[]) { const items=requireItems(diverseSmallItems(4,excluded,item=>item.price>=20&&item.price<=500,liveItems),4,"Shell Game small-prize bank"), shown=items.map((x,i)=>x.price+(i%2?-8:9)), bonus=pick(requireItems(grandPrizeCandidates(excluded,liveItems),1,"Shell Game grand-prize bank")),bonusPrize={...prizeIntro(bonus),announcerText:`You could win this spectacular grand prize: ${bonus.description}`}; return {...base("shellGame","SHELL GAME",player,"Price four small prizes. Every correct higher-or-lower decision wins that prize and earns a marker for one shell. Then choose from the shells you earned. Find the hidden ball to win the grand prize. Earn all four shells and the grand prize is guaranteed.",[bonusPrize,...items.map(prizeIntro)]),bonusPrize,_bonusPrice:bonus.price,items:items.map((x,i)=>({...prizeIntro(x),shownPrice:shown[i]})),_prices:items.map(i=>i.price),itemIndex:0,shells:0,wonSmallValue:0,_ball:Math.floor(Math.random()*4),chosenShells:[],prompt:`${items[0].name}: higher or lower than ${money(shown[0])}?`,mode:"choice",options:["Higher","Lower"],stage:"prices"}; }
...
function makeDoublePrices(player,excluded=[],liveItems=[]){const raw=pick(requireItems(grandPrizeCandidates(excluded,liveItems),1,"Double Prices bank")),prize=prizeIntro(raw),wrong=raw.price+pick([650,800,950,1200]),options=shuffle([raw.price,wrong]);return {...base("doublePrices","DOUBLE PRICES",player,"The grand prize has two possible prices. Choose the correct price to win it.",[prize]),featuredIntroCount:1,prize,_actual:raw.price,prices:options,prompt:"Which is the correct price?",mode:"choice",options:options.map(money)};}
...
function makePickAPair(player,excluded=[],liveItems=[]){
  ...
  const rawBonus=pick(requireItems(grandPrizeCandidates(excluded,liveItems),1,"Pick-a-Pair grand-prize bank")),bonusPrize={...prizeIntro(rawBonus),announcerText:`Match two grocery prices and you could win this fabulous prize: ${rawBonus.description}`};
