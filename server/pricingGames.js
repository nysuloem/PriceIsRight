...
const GAME_NAMES = ["plinko", "cliffHangers", "punchABunch", "diceGame", "groceryGame", "oneAway", "clockGame", "anyNumber", "grandGame", "shellGame", "moneyGame", "luckySeven"];
...
const carIntro = (p) => ({...prizeIntro(p),announcerText:`IT'S A NEW CAR!!! ${p.description}`});
...
function makeClock(player,excluded=[]) { const items=shuffle(fresh(SMALL_ITEMS.filter(i=>i.price>=25),excluded)).slice(0,2); return {...base("clockGame","CLOCK GAME",player,"You have 90 seconds to price two prizes. After every guess, the host tells you higher or lower.",items.map(prizeIntro)),items:items.map(prizeIntro),_prices:items.map(i=>Math.round(i.price)),itemIndex:0,_startedAt:null,_clockStarted:false,prompt:`Guess the price of the ${items[0].name}.`,mode:"number",clue:"90 seconds starts now!",secondsLeft:90}; }
...
const FACTORIES={plinko:makePlinko,cliffHangers:makeCliff,punchABunch:makePunch,diceGame:makeDice,groceryGame:makeGrocery,oneAway:makeOneAway,clockGame:makeClock,anyNumber:makeAnyNumber,grandGame:makeGrand,shellGame:makeShell,moneyGame:makeMoneyGame,luckySeven:makeLuckySeven};
