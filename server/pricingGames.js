...
function makeMoneyGame(player,excluded=[]) { const car=pick(fresh(CARS,excluded)),digits=String(car.price),front=digits.slice(0,2),back=digits.slice(-2),decoys=new Set([front,back]);while(decoys.size<9)decoys.add(String(10+Math.floor(Math.random()*90)));const options=shuffle([...decoys]);return {...base("moneyGame","MONEY GAME",player,"Pick the first two and last two numbers in the car's price. Wrong choices go on your cash line; four wrong choices end the game.",[carIntro(car)]),car:prizeIntro(car),_digits:digits.split("").map(Number),_front:front,_back:back,frontFound:false,backFound:false,cash:0,wrong:[],choices:options,prompt:"Pick a number pair.",mode:"choice",options}; }
