...
test("Shell Game asks for another shell and exposes the ball for the final lift",()=>{
  const g=createPricingGameForType("shellGame",player);g.stage="shell";g.shells=2;g.chosenShells=[];g.options=["1","2","3","4"];g._ball=3;
  playPricingGame(g,{choice:"1"});assert.equal(g.prompt,"Pick another shell.");
  playPricingGame(g,{choice:"2"});assert.equal(g.status,"lost");assert.equal(g.revealedBall,4);
});
