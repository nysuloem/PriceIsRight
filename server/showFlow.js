...
const controls=(contestant,playerId)=>contestant?.id===playerId||contestant?.controllerPlayerId===playerId;
...
export function showcaseAction(f,playerId,action){const high=f.contestants[0],low=f.contestants[1];if(f.stage==="choice"){if(!controls(high,playerId))throw new Error("Only the top winner may bid or pass");if(action.choice==="pass"){f.assignments[0]=low.id;f.assignments[1]=high.id;}else{f.assignments[0]=high.id;f.assignments[1]=low.id;}f.stage="firstBid";return;}
  if(f.stage==="firstBid"||f.stage==="secondBid"){const index=f.stage==="firstBid"?0:1,contestant=f.contestants.find(c=>c.id===f.assignments[index]);if(!controls(contestant,playerId))throw new Error("This is not your showcase");const bid=Math.max(1,Math.round(Number(action.bid)||0));f.bids[contestant.id]=bid;if(index===0){f.showcaseIndex=1;f.stage="secondTheme";}else revealShowcases(f);return;}throw new Error("Showcase is not accepting that action");}
