...
export function PricingGameView({ game, spotlight = null, rulesOnly = false }) {
  ...
  return (
    <div className={`pir-pricing-board pir-game-${game.type}`}>
      ...
      {game.type === "shellGame" && <><div className="pir-shell-bonus"><b>MAJOR BONUS PRIZE</b><GameCards items={[game.bonusPrize].filter(Boolean)} /></div><div className="pir-shell-status">SHELL MARKERS EARNED: {game.shells} · SMALL PRIZES WON: ${Number(game.wonSmallValue||0).toLocaleString("en-CA")}</div><div className="pir-shell-board">{[1,2,3,4].map(n=><div key={n} className={game.revealedBall===n?"ball-revealed":game.chosenShells?.includes(n)?"shell-lifted":""}><span className="pir-shell-cup">🐚</span><i>{game.revealedBall===n?"●":""}</i><b>{n}</b></div>)}</div>{game.stage==="prices"&&<><div className="pir-small-prize-label">SMALL PRIZE {game.itemIndex+1} OF 4</div><GameCards items={[game.items[game.itemIndex]].filter(Boolean)} /></>}</>}
