...
export function PricingGameView({ game, spotlight = null, rulesOnly = false }) {
  ...
  return (
    <div className={`pir-pricing-board pir-game-${game.type}`}>
      ...
      {game.type === "comingOrGoing" && <><GameCards items={[game.prize]} /><div className="pir-coming-going"><b>COMING →</b><span>${Number(game.prices[0]).toLocaleString("en-CA")}</span><span>${Number(game.prices[1]).toLocaleString("en-CA")}</span><b>← GOING</b></div></>}
